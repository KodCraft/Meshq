...
class SendSubscriptionRequestUseCase @Inject constructor(private val repository: TrainerRepository) :
    BaseUseCase<String, Response<Boolean>> {
    override suspend fun execute(param: String): Flow<Response<Boolean>> =
        repository.sendSubscriptionRequest(trainerId = param, traineeId = UserManager.getUserId())
}